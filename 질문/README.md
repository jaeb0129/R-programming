...
12. RSE(residual standard error)
- 잔차표준오차(residual standard error)는 관측값들이 추정된 회귀직선의 주위에 흩어져 있는 정도를 나타낸다.
- 잔차표준오차 s의 값이 작을수록 관측값들이 추정된 회귀직선에 근접해 있음을 나타낸다.

13. R-squared(결정계수)
- 결정계수 R^2는 총 변동 중에 회귀 모형이 설명하는 변동을 나타내는 측도로 회귀직선이 관측값을 얼마나 잘 설명하는지 나타낸다
- 즉, 결정계수는 총변동 중에 회귀직선으로 설명된 변동이 차지하는 비를 나타내며 값이 1에 가까울수록 회귀식이 관측값들을 잘 설명하고 있다는 것을 뜻한다.

14. Adjusted R-squared
- 결정계수 R^2는 그 의미가 명확하고 해석이 쉽기 때문에 널리 사용된다.
- 하지만, 회귀모형이 점점 복잡해질수록 (설명변수의 수가 많아질수록) SSE는 작아지고 SSR은 커지기 때문에 R^2값이 항상 증가한다.
- 실제 Y 에 관련되어 있지 않은 변수가 모형에 포함되어도 R^2값은 항상 커지므로 때로는 적절하지 않은 측도가 될 수 있다.
- 따라서 이러한 단점을 보완한 수정된 결정계수 (adjusted R^2)를 사용한다.
